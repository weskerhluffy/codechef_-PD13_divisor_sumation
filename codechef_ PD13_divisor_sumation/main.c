...
typedef struct sumas_divisores{
    hm_rr_bs_tabla contribuciones_primos_memoria[PD13_MAX_PRIMO+1];
    hm_rr_bs_tabla *contribuciones_primos[PD13_MAX_PRIMO+1];
    entero_largo_sin_signo sumas_divisores[PD13_MAX_PRIMO+1];
    entero_largo_sin_signo factores_primos[PD13_MAX_PRIMO+1][PD13_MAX_FACTORES_PRIMOS];
    entero_largo_sin_signo factores_primos_cnt[PD13_MAX_PRIMO+1];
}sumas_divisores;
...
COMUN_FUNC_STATICA void pd13_main(){
    ...
    for(t=0;t<=PD13_MAX_PRIMO;t++){
        sd->contribuciones_primos[t]=&(sd->contribuciones_primos_memoria[t]);
        hash_map_robin_hood_back_shift_init(sd->contribuciones_primos[t], PD13_MAX_FACTORES_PRIMOS<<1);
    }
